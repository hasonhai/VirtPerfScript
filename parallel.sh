#!/bin/bash
# A script from kawakamasu, pebblesinthesand.wordpress.com
# With minor modifying by Hai S. Ha (hasonhai124(at)gmail.com)
NUM=0
QUEUE=""
MAX_NPROC=2 # default
...
Usage: `basename $0` [-h] [-r] [-j nb_jobs] command [arg_list]
 ...
 If arg_list is existed, its items are distributed to each process to devide jobs.
 Examples:
...
if [ $# -eq 0 ]; then # there is no [arg_list]
    for INCR in $(seq 1 $MAX_NPROC)
    do
        CMD="$COMMAND"; 
        echo "Running $CMD"
        $CMD &
